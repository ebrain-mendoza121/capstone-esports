...
def normalize_game_duration(match_info: Dict[str, Any]) -> int:
    ...
    Args:
        match_info: The "info" section from match JSON
        
    Returns:
        Game duration in seconds
        
    Reference:
        https://developer.riotgames.com/docs/lol#match-v5_getMatch
    """
    game_duration = match_info.get("gameDuration", 0)
    game_end_timestamp = match_info.get("gameEndTimestamp")
    
    # If gameEndTimestamp exists, duration is already in seconds (patch 11.20+)
    if game_end_timestamp is not None:
        return int(game_duration)
    
    # Otherwise, duration is in milliseconds (pre-patch 11.20)
    # Convert to seconds
...
def compute_derived_metrics(
    ...
    Args:
        participant: The participant's data from Riot API
        team_participants: All participants on the same team
        game_duration_seconds: Match duration in seconds (normalized)
        
    Returns:
        Dictionary with computed metrics (all nullable floats)
    ...
    # KDA: if deaths = 0, use 1 as denominator (standard convention)
    ...
    # If team has 0 kills, participation is 0.0
    kill_participation = (
        (kills + assists) / team_kills if team_kills > 0 else 0.0
    )
    
    # Damage share: player_damage / team_damage
    # If team has 0 damage, share is 0.0
    damage_share = (
        total_damage / team_damage if team_damage > 0 else 0.0
    )
    ...
    return {
        ...
        "kill_participation": round(kill_participation, 4),  # 0.0 to 1.0 scale
        "damage_share": round(damage_share, 4),  # 0.0 to 1.0 scale
...
def extract_team_participants(
    ...
    Args:
        all_participants: All participants in the match
        team_id: Team ID (typically 100 or 200)
        
    Returns:
        List of participants on the specified team
    """
