...
def get_player_metrics(session: Session, puuid: str):
    ...
    for ps, match in stats:
        minutes = match.game_duration / 60 if match.game_duration else 0
    ...
    return {
        ...
        "kda": round(kda, 2),
        "cs_per_min": round(total_cs / total_minutes, 2) if total_minutes else 0,
        "gold_per_min": round(total_gold / total_minutes, 2) if total_minutes else 0,
        "vision_per_min": round(total_vision / total_minutes, 2) if total_minutes else 0,
    }